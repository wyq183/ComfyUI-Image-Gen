...
(function () {
  ...
  function GenPanel() {
    ...
    function loadImages(targetCategory) {
      ...
      req('/images?category=' + encodeURIComponent(selected) + '&sort=' + encodeURIComponent(gallerySort[0]) + '&limit=60&offset=0&_=' + Date.now()).then(function (d) {
        // 忽略较早请求的迟到响应，防止切换分类后又被旧分类结果覆盖。
        if (requestId === galleryRequest.current) { imgs[1](d.items || []); galleryTotal[1](Number(d.total || 0)); galleryOffset[1](0); }
    ...
    function loadMoreImages() { if (galleryLoading[0] || imgs[0].length >= galleryTotal[0]) return; galleryLoading[1](true); var next=galleryOffset[0]+60; req('/images?category='+encodeURIComponent(category[0])+'&sort='+encodeURIComponent(gallerySort[0])+'&limit=60&offset='+next+'&_='+Date.now()).then(function(d){ imgs[1](imgs[0].concat(d.items||[])); galleryOffset[1](next); galleryTotal[1](Number(d.total||galleryTotal[0])); }).catch(function(e){message.error(e.message||'加载更多失败');}).then(function(){galleryLoading[1](false);}); }
    ...
    function scanGallery() { galleryLoading[1](true); req('/gallery/scan', { method: 'POST' }).then(function (d) { if(!d.task_id){ message.success(d.message || '扫描完成'); galleryLoading[1](false); loadCategories(); loadImages(); return; } message.info('图库正在后台扫描，可继续使用其他功能'); function pollScan(){ req('/tasks/'+d.task_id+'?_='+Date.now()).then(function(t){ if(t.state==='queued'||t.state==='running'){ window.setTimeout(pollScan,1200); return; } galleryLoading[1](false); if(t.state==='completed'){ message.success(t.message || '图库扫描完成'); loadCategories(); loadImages(); } else message.error((t.failures&&t.failures[0]) || t.message || '图库扫描失败'); }).catch(function(e){ galleryLoading[1](false); message.error(e.message || '读取扫描进度失败'); }); } pollScan(); }).catch(function (e) { galleryLoading[1](false); message.error(e.message); }); }
    React.useEffect(function () {
      load();
      loadCategories();
      loadImages();
    ...
    function upscaleGalleryImage(img) { if(!img) return; upscaleGalleryIds[1]([img.id]); upscaleCategory[1](img.category || upscaleCategory[0]); preview[1](null); tab[1]('upscale'); message.success('已选择这张图片，已自动打开「放大」设置'); }
    function goGalleryForUpscale() { tab[1]('gallery'); batchMode[1](true); message.info('请在图库中勾选图片，然后点击「加入放大队列」'); }
    function submitGalleryUpscale() { var ids=upscaleGalleryIds[0]; if(!ids.length) return message.warning('请先从图库选择图片'); req('/upscale/gallery/batch?profile='+encodeURIComponent(upscaleProfile[0])+'&category='+encodeURIComponent(upscaleCategory[0]), {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image_ids:ids})}).then(function(r){ task[1]({id:r.task_id,kind:'upscale',state:'queued',total:r.total,completed:0,message:'批量放大任务已提交'}); pollUpscaleTask(r.task_id); }).catch(function(e){message.error(e.message || '提交批量放大失败');}); }
    ...
    return h(React.Fragment, null,
      ...
      tab[0] === 'upscale' ? h('div',{style:{flex:1,overflowY:'auto'}},
        h(Section,{title:'图片放大'},
          ...
          h('div',{style:{display:'flex',gap:6,alignItems:'center',margin:'4px 0 10px'}},h(Button,{size:'small',onClick:goGalleryForUpscale},'从图库选择图片'),h('span',{style:{fontSize:11,color:'var(--ant-color-text-secondary)'}},upscaleGalleryIds[0].length?'已选 '+upscaleGalleryIds[0].length+' 张':'未选择图库图片')),
          ...
          upscaleGalleryIds[0].length ? h(Button,{type:'primary',block:true,onClick:submitGalleryUpscale,disabled:!upscaleProfiles[0].length},'开始批量放大（'+upscaleGalleryIds[0].length+' 张）') : h(Button,{type:'primary',block:true,onClick:doUpscaleUpload,disabled:!upscaleFile[0] || !upscaleProfiles[0].length},'开始放大')
        ),
      ...
      tab[0] === 'gallery' ? h(React.Fragment, null,
        ...
        batchMode[0] ? h('div', { style: { padding: '6px 8px', display: 'flex', gap: 6, alignItems: 'center', borderBottom: '1px solid var(--border-color-split)', background: 'var(--ant-color-fill-quaternary)' } },
          ...
          h(Button, { size: 'small', danger: true, disabled: batchBusy[0] || !selectedIds[0].length, onClick: batchUpscale }, '加入放大队列'), h(Button, { size: 'small', danger: true, disabled: batchBusy[0] || !selectedIds[0].length, onClick: batchDelete }, '删除')
        ) : null,
        h('div', { style: { flex: 1, overflowY: 'auto', padding: 8, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, alignContent: 'start' } },
          ...
          galleryItems().length ? galleryItems().map(function (img) {
            ...
            return h('div', { key: img.id, onClick: function () { batchMode[0] ? toggleSelected(img.id) : preview[1](img.id); }, style: { position: 'relative', aspectRatio: '1/1', border: checked ? '2px solid var(--ant-color-primary)' : '1px solid var(--border-color-split)', borderRadius: 6, overflow: 'hidden', cursor: 'pointer', boxSizing: 'border-box' } },
              h('img', { src: iurl(img.id), loading:'lazy', style: { width: '100%', height: '100%', objectFit: 'cover' } }),
      ...
      galleryTotal[0] > imgs[0].length ? h('div',{style:{padding:'8px',textAlign:'center'}},h(Button,{size:'small',loading:galleryLoading[0],onClick:loadMoreImages},'加载更多（已显示 '+imgs[0].length+' / '+galleryTotal[0]+'）')) : null,
      ...
      preview[0] ? (function () { var img = (imgs[0] || []).find(function (x) { return x.id === preview[0]; }); if (!img) return null;
        ...
        return h(Modal, { open: true, footer: null, width: 560, onCancel: function () { preview[1](null); },
          ...
          h('img', { src: iurl(img.id), loading:'lazy', style: { width: '100%', borderRadius: 8, marginBottom: 12 } }),
