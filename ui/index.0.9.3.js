...
(function () {
  ...
  function GenPanel() {
    ...
    var state = s(null), model = s(''), tab = s('gen'), imgs = s([]), preview = s(null), busy = s(false), scanning = s(false), galleryLoading = s(false), category = s('未分类'), categories = s(['未分类']);
    var galleryRequest = { current: 0 };
    ...
    function switchCategory(v) { category[1](v); loadImages(v); }
    ...
    return h(React.Fragment, null,
      ...
      tab[0] === 'gallery' ? h(React.Fragment, null,
        h('div', { style: { padding: 8, display: 'flex', gap: 6, borderBottom: '1px solid var(--border-color-split)' } },
          h(Select, { size: 'small', value: category[0], style: { flex: 1 }, options: [{ value: '', label: '全部分类' }].concat(categories[0].map(function (x) { return { value: x, label: x }; })), disabled: galleryLoading[0], onChange: switchCategory }),
          h(Button, { size: 'small', onClick: scanGallery }, '扫描 ComfyUI'),
          h(Button, { size: 'small', onClick: function () { var n = window.prompt('新建分类名称'); if (!n || !n.trim()) return; req('/gallery/categories/create?name=' + encodeURIComponent(n.trim()), { method: 'POST' }).then(function () { loadCategories(); switchCategory(n.trim()); }).catch(function (e) { message.error(e.message); }); } }, '+ 分类')
        ),
